...
# Общий класс для игрока и компьютерного игрока
class WOFPlayer:
...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        Guess a letter, phrase, or type 'exit' or 'pass':""".format(self.name, self.prizeMoney,
                                                                    category, obscuredPhrase, guessed))
        return input()


# Сабкласс для игрока-компьютера
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        # Helps deciding if move is goor or bad (for computer player)
        return True if random.randint(1, 10) > self.difficulty else False

    def getPossibleLetters(self, guessed):
        # Returns list of letters that can be guessed
        possible = []
